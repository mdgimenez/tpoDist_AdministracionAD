...
public class UnidadDAO {
	...
	public Unidad findById(int identificador) throws EdificioException, UnidadException {
		Unidad resultado = null;
		SessionFactory sf = HibernateUtil.getSessionFactory();
		Session s = sf.getCurrentSession();
		s.beginTransaction();
		UnidadEntity unidad = (UnidadEntity) s.createQuery("from UnidadEntity u where u.identificador = ?")
	...
	Unidad toNegocio(UnidadEntity e) throws EdificioException, UnidadException {
		if(e != null) {
			...
			Unidad unidad = new Unidad(e.getIdentificador(), e.getPiso(), e.getNumero(), edificio);
	...
	UnidadEntity toEntity(Unidad u) {
		return new UnidadEntity(u.getId(), u.getPiso(), u.getNumero(), EdificioDAO.getInstancia().toEntity(u.getEdificio()));
	}
